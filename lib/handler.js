...
async function handler(event) {
    // extract callback config from params
    const { callbackUrl, callbackToken, ...configParams } = event
    // override base config with provided params
    const baseConfig = { ...DEFAULT_CONFIG, ...configParams }
    const config = event.USE_IAM_AUTH === true ? decorateWithIamToken(baseConfig) : baseConfig
    try {
        const result = await backup(config)

        console.log("CALLBACK_CONFIG", callbackUrl, callbackToken)
        // TODO: validate callback uri (only allowed origins) => maybe add Regex as env var
        ...
        const response = await fetch(
            callbackUrl, 
            {
                ...
                body: JSON.stringify({
                    "key": result.Key,
                    "byte_size": 123, // TODO: add real file size
                    "checksum": "asdf" // TODO: add real checksum
                }),
                headers: {
        ...
        // TODO: expect 2xx response
        console.log("CALLBACK_STATUS", response.status)
        const jsonResponse = await response.text();//assuming data is json
        console.log("CALLBACK_BODY", jsonResponse)
